...
# I made a histogram to see how commonly elevators are used across apartment buildings and to check that most values are not just 0.
# It seems like most apartments do not have elevators.
# There also seems to be an outlier with more than 30 elevators in the building I want to filter out that # building to check if everything else looks reasonable
filter(apt_buildings, no_of_elevators > 30)
# apt building ID 11116 has 32 elevators, 6 storeys, and 68 units. Maybe this building just has a weird design.
```
...
units_storeys <- apt_buildings %>% 
  ...
  geom_point(size = 0.5)
print(units_storeys)
# I created a scatterplot to see if there is a relationship between the number of units and the number of storeys in a building that might also explain the number of elevators in a building. However, there seems to be an outlier.
...
3. Are the wards equally distirbuted?
